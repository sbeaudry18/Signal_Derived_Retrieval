function [ fname ] = sp_savename_tempo( date_in, region, instrument, ext, behr_version, scan_num, gran_num )
%SP_SAVENAME Create the file name for an SP file of a given date
%   Last modified for BEHR-RED by Sam Beaudry, 2025-02-01. Added orbit
%   number (since current operations of BEHR-RED are on an orbit basis) and
%   processing time to avoid overwriting existing files.
%
...
savetime = datetime('now', 'TimeZone', 'UTC', 'Format', 'yyyyMMdd''T''HHmmss');

if isempty(region)
    fname = sprintf('%s_SP_%s_%s_S%sG%s_proc_%s.%s', instrument, ver_str, datestr(date_in, 'yyyymmdd'), scan_num, gran_num, savetime, ext);
else
    fname = sprintf('%s_SP_%s_%s_%s_S%sG%s_proc_%s.%s', instrument, upper(region), ver_str, datestr(date_in, 'yyyymmdd'), scan_num, gran_num, savetime, ext);
end
