...
4. Navigate to the "constants" folder and call 'python setup_paths.py'. This will create a path file, "sdr_paths.config", which points to the TEMPO L2 files to be processed and sets the location SDR files will be saved.
5. The main script to run the SDR is "SDR-director-simple.sh" in the "TEMPO/main" folder. Under "Configure Environment", enter the commands needed to use the above packages. Some of the additional options for running SDR are listed below. The suggested values are those used to process the data presented in Beaudry and Cohen (2026):
...
6. Once "SDR-director-simple.sh" is setup, use `bash SDR-director-simple.sh` to start the algorithm.
